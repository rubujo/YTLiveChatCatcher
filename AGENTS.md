...
- **純工具函式**：`GetYouTubeChannelID`／`GetYouTubeVideoID`／`GetYouTubeChannelUrl`（`Utils/YouTubeUrlUtil.cs`）都是不依賴 instance 狀態的 `static` 方法，呼叫時不需要（也不應該透過）`YTJsonParser` 實例。**`ParseYouTubeChannelID`（`@handle`／`/c/`／`/user/` 網址轉頻道 ID）靠抓取頁面找 `og:url` 這個 meta 標籤取得真正的頻道 ID，不要假設它一定是 `<head>` 的直接子節點**——實測發現部分頻道（例如 `@handle` 網址）的頁面現在會把這類 SEO meta 標籤放在 `<body>` 底下，原本用 `document.Head.Children.FirstOrDefault(...)` 只找直接子節點，遇到這種頁面會靜默失敗（`catch` 吞掉、回傳空字串），讓 WinForms 端「自動取得頻道 ID」整個功能看起來像壞掉（輸入框內容原封不動）；已改用 `document.QuerySelector("meta[property='og:url']")` 搜尋整份文件。找 meta／SEO 標籤這類 YouTube 頁面內容時，不要假設它固定在某個容器底下，用 `QuerySelector`／`QuerySelectorAll` 掃描整份文件比較穩妥。找 meta 標籤內容的核心邏輯抽到 `YouTubeUrlUtil.GetOgUrlContent(IDocument?)`（純函式，不碰網路），由 `YTJsonParser.Tests/YouTubeUrlUtilTests.cs` 用 AngleSharp 直接解析的靜態 HTML 字串覆蓋（含 og:url 在 `<body>` 底下的回歸案例），不需要真的連線到 YouTube。
